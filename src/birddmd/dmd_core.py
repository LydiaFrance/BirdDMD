...
from typing import Optional, Tuple, Union, Dict, Any, Set
import warnings
...
from .exceptions import ValidationError, ComputationError
...
def _compute_dmd(data: np.ndarray,
                times: np.ndarray,
                n_modes: int,
                eig_constraints: Set[str],
                d: int) -> Tuple[Optional[BOPDMD], np.ndarray]:
    ...
    try:
        ...
        raise ComputationError(f"DMD computation failed: {str(e)}")

def reorder_dmd_results(dmd_results: BOPDMD,
                       num_markers: int,
                       n_modes: int,
                       order_by: str = 'amplitude',
                       reshape_modes: bool = True) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
...
def run_dmd(data: np.ndarray, 
    ...
    if not is_timeseries:
        if num_markers is None:
            raise ValidationError("num_markers must be provided for marker data")
    ...
    try:
        ...
        # Import reconstruct_dmd from dmd_reconstruction
        ...
        raise ComputationError(f"DMD analysis failed: {str(e)}")

def run_sequence_dmd(bird_name: str, 
                    perch_dist: str, 
                    turn: str, 
                    behaviour: str, 
                    seqID: str, 
                    ...
                    d: int = DEFAULT_D, 
                    eig_constraints: Set[str] = DEFAULT_EIG_CONSTRAINTS,
                    min_seq_length: Optional[int] = DEFAULT_MIN_SEQ_LENGTH, 
                    interpolate: bool = False, 
                    verbose: bool = True) -> Tuple[Optional[np.ndarray], ...]:
    ...
    Returns:
        Tuple of (times, markers, Lambda, Modes, bn, Psi, phase_shifts, dmd_results, keypoints)
    ...
    try:
        # Load data
        from .data_handling import load_bird_data
        ...
        # Load sequence data
        from .data_handling import load_sequence_data
        ...
        if interpolate:
            from .data_handling import spline_interpolation
        ...
        raise ComputationError(f"Sequence DMD analysis failed: {str(e)}")

def run_marker_dmd(markers: np.ndarray, 
                  times: Optional[np.ndarray] = None, 
                  average_shape: Optional[np.ndarray] = None, 
                  n_modes: int = DEFAULT_N_MODES, 
                  d: int = DEFAULT_D,
    ...
    if len(markers.shape) != 3:
        raise ValidationError("Markers must be 3D array (n_frames, n_markers, 3)")
...
def run_timeseries_dmd(data: np.ndarray, 
                      times: Optional[np.ndarray] = None, 
                      n_modes: int = DEFAULT_N_MODES, 
                      d: int = DEFAULT_D,
                      eig_constraints: Set[str] = DEFAULT_EIG_CONSTRAINTS, 
    ...
    """
    if len(data.shape) != 2:
        raise ValidationError("Data must be 2D array (n_timesteps, n_variables)")
...
def dmd_loop_seqs(bird_name: str,  
                 save_path: str,
                 behaviour: str,
    ...
    try:
        ...
        for seqID in df['seqID'].unique():
            times, markers, Lambda, Modes, bn, Psi, phase_shifts, dmd_results, keypoints = run_DMD_sequence(
                seqID, df, marker_column_names, average_shape, n_Modes, min_seq_length, eig_constraints, d
            )
            if times is not None:
                save_sequence_results(save_path, seqID, times, markers, Lambda, Modes, bn, Psi, keypoints)

    except Exception as e:
        raise ComputationError(f"Batch DMD analysis failed: {str(e)}")
...
def run_DMD_sequence(seqID: str, 
                    ...
                    eig_constraints: Set[str] = DEFAULT_EIG_CONSTRAINTS, 
                    d: int = DEFAULT_D, 
                    interpolate: bool = False) -> Tuple[Optional[np.ndarray], ...]:
    """
    ...
    Returns:
        Tuple of (times, original_markers_reshaped, Lambda, Modes, bn, Psi, phase_shifts, dmd_results, keypoints)
